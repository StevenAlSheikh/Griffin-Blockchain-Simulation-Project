...
public class Block {
    ...
    public synchronized String computeMerkleRoot(ArrayList<String> lstItems) {
        ...
        if (lstItems.size() == 2) {
            MerkleNode[] aiNodes = new MerkleNode[3];
            for (int i = 0; i < aiNodes.length; i++)
            {
                aiNodes[i] = new MerkleNode();
            }
            for (int i = 0; i < 2; i++)
            {
                aiNodes[i].sHash = BlockchainUtil.generateHash(lstItems.get(i));
            }
            populateMerkleNode(aiNodes[2], aiNodes[0], aiNodes[1]);

            return aiNodes[2].sHash;
        }
        else {
            MerkleNode[] aiNodes = new MerkleNode[7];
            for (int i = 0; i < aiNodes.length; i++) {
                aiNodes[i] = new MerkleNode();
            }
            for (int i = 0; i < 4; i++) {
            ...
            populateMerkleNode(aiNodes[4], aiNodes[0], aiNodes[1]);
            populateMerkleNode(aiNodes[5], aiNodes[2], aiNodes[3]);
            populateMerkleNode(aiNodes[6], aiNodes[4], aiNodes[5]);
            return aiNodes[6].sHash;
        }
